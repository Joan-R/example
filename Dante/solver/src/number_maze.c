...
{
    int row = solvtab->row, col = solvtab->col, i = 0;
...
void clos(open_t **head, open_t **tail, int small, tab *solvtab)
{
    int i = 0, end = 0;
    open_t *ptr = *head;
    solvtab->row = ptr->node->row;
    solvtab->col = ptr->node->col;
    set_add_c(tail, ptr->node, solvtab);
    int row = solvtab->row, col = solvtab->col;
    ...
    i += clos_next(head, solvtab, end, ptr);
    del_node(&(*head), 0);
}
